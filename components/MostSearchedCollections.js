...
const MostSearchedCollections = () => {
	return (
		<div className="max-w-[1980px] mx-auto py-2 px-4 md:px-6">
			<h2 className="text-big font-medium mb-4">Most searched</h2>
			<ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 bg-white border border-slate-200 rounded-sm">
				{collections.map((collection, index) => (
					<li
						key={index}
						className="w-full flex items-center justify-center text-center p-8 border-l border-t border-slate-200"
					>
						<Link href={`/search?s=${collection.slug}`} className="relative group px-2">
